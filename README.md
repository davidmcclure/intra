...
This approach has couple of advantages:
  ...
  1. **No need to choose a segment length** - The whole concept of a "document" index breaks down when the object of analysis _is itself a document_ (or a set of documents that's treated as a single, composite document - e.g., the collected works of Shakespeare). It's impossible to pick a single segment length that's optimal for all queries - relevance doesn't distribute in evenly-sized chunks inside of texts, meaning that there's no single "unit" to return as a result. Trying to pick one will always favor one type of result over another - if you index a text as a series of sentences, the search results won't represent high-level patterns in meaning distribution at the level of chapters or sections (e.g., war vs. peace in _War and Peace_). But if you index large sub-documents, you iron over important fluctuations at the sentence / phrase level.
